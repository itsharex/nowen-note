...
2. 只上传 md/txt/html 文本内容
3. PDF/DOCX 暂不作为附件上传
4. 本地删除不会删除 Nowen 笔记（标记 deleted）
5. 自动同步仅在 App 运行且用户已登录时生效
6. 不做系统后台服务、不做 watcher、不做开机自启
7. 同步是单向的：本地 → Nowen，不支持反向同步

## 后续计划

- DESKTOP-FOLDER-KB-SYNC-02：PDF/DOCX 附件上传或文本提取
- 可选：双向同步 / 冲突合并
- 可选：文件系统监听（chokidar）
